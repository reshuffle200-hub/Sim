 ...
 * Radiation board.
 *
 * Laid out as the DIAGNOSTIC works, left to right: the source term, then the
 * paths activity can take out of it, then where it ends up. The off-gas monitor
 * is given its own line because it answers a different question from the
 * per-generator monitors -- it says leakage exists, they say which unit -- and
 * conflating those two is how you isolate the wrong generator.
 */
export function radiation(PL) {
  const R = PL.rd, rp = PL.rdp;
  const sus = R.steamline.reduce((b, m, i) =>
    Math.max(m.r, R.blowdown[i].r) > Math.max(R.steamline[b].r, R.blowdown[b].r) ? i : b, 0);
  const leaking = R.sgLeakGpm.some(v => v > 0);
  const mon = m => (m.offScale ? '> full scale' : F(m.r, 1) + '×');
  const st = m => m.hihi ? 'bad' : (m.alarm ? 'warn' : 'good');

  return `<div class="grid3">
  ${W.card('Primary source term', `<div class="rows">${W.rows([
    ['RCS activity', F(R.primaryUCiG, 2) + ' µCi/g',
      R.primaryUCiG > rp.primarySpec ? 'bad'
        : (R.primaryUCiG > rp.primaryNormal * 10 ? 'warn' : 'good')],
    ['Tech Spec limit', F(rp.primarySpec, 0) + ' µCi/g', 'dimv'],
    ['Noble gas', F(R.gasUCiG, 1) + ' µCi/g',
      R.alarms.gasHi ? 'warn' : 'dimv'],
    ['Clad damage', R.cladDamage > 1e-6 ? F(R.cladDamage * 100, 3) + ' %' : 'none',
      R.cladDamage > 1e-4 ? 'bad' : 'good'],
    ['Letdown monitor', mon(R.letdown), st(R.letdown)]
  ])}</div>`, R.primaryUCiG > rp.primaryNormal * 10 ? 'elevated' : 'intact fuel')}

  ${W.card('Primary → secondary', `<div class="rows">${W.rows([
    ['Off-gas monitor', mon(R.offgas), st(R.offgas)],
    ['Leakage indicated', leaking ? 'YES' : 'no', leaking ? 'bad' : 'good'],
    ['Total leak rate', F(R.sgLeakGpm.reduce((a, b) => a + b, 0), 2) + ' gpm',
      leaking ? 'bad' : 'dimv'],
    ['Suspect generator', Math.max(R.steamline[sus].r, R.blowdown[sus].r) > 3
      ? 'SG ' + 'ABC'[sus] : '—',
      Math.max(R.steamline[sus].r, R.blowdown[sus].r) > 3 ? 'bad' : 'dimv'],
    ['Blowdown path', R.blowdownIsolated ? 'ISOLATED' : 'sampling',
      R.blowdownIsolated ? 'bad' : 'good']
  ])}</div>`, 'ejector concentrates ×' + F(rp.bgSteamUCiG / rp.bgOffgasUCiG * 1000, 0))}

  ${W.card('Effluent and area', `<div class="rows">${W.rows([
    ['Plant vent', mon(R.vent), st(R.vent)],
    ['Release rate', R.releaseRateUCiS > 1
      ? F(R.releaseRateUCiS / 1e3, 2) + ' mCi/s' : 'none',
      R.alarms.releasing ? 'bad' : 'good'],
    ['Released', F(R.releasedCi, 3) + ' Ci', R.releasedCi > 1 ? 'warn' : 'dimv'],
    ['CR intake', mon(R.crIntake), st(R.crIntake)],
    ['Control room', R.crIsolated ? 'ISOLATED' : 'normal',
      R.crIsolated ? 'bad' : 'good'],
    ['Aux building', mon(R.auxBuilding), st(R.auxBuilding)],
    ['Fuel handling', mon(R.fuelHandling), st(R.fuelHandling)]
  ])}</div>`)}
  </div>

  <div class="grid3">
  ${W.card('Steam generator monitors', `<div class="rows">${W.rows(
    ['A', 'B', 'C'].flatMap((L, i) => [
      [`SG ${L} blowdown`, mon(R.blowdown[i]), st(R.blowdown[i])],
      [`SG ${L} steam line`, mon(R.steamline[i]), st(R.steamline[i])]
    ]))}</div>`, 'these identify the unit')}

  ${W.card('Containment monitors', `<div class="rows">${W.rows([
    ['Gas channel', mon(R.cnmtGas), st(R.cnmtGas)],
    ['Particulate', mon(R.cnmtPart), st(R.cnmtPart)],
    ['Iodine', mon(R.cnmtIodine), st(R.cnmtIodine)],
    ['High range', R.cnmtHighRange > 0.05
      ? F(R.cnmtHighRange, 2) + ' R/hr' : 'background',
      R.cnmtHighRange > 1 ? 'bad' : 'good'],
    ['CCW loop', F(PL.cw.activityUCiMl * 1e3, 3) + ' nCi/ml',
      R.alarms.ccwActivity ? 'bad' : 'good']
  ])}</div>`)}

  ${W.card('Reading the monitors', `<div class="note">
    Off-gas is the <b>most sensitive</b> path and the <b>least specific</b>. The air
    ejector pulls non-condensables into a stream four orders of magnitude smaller
    than the steam flow, so it concentrates them by the same factor and responds
    to a few hundredths of a gallon a minute within seconds — but it sees all
    three generators mixed together and cannot tell you which one.
    Blowdown and the steam line monitors are slower, because iodine has to build
    a liquid concentration first, and they are per-unit — so they are what
    identifies the generator you are going to isolate.
    ${R.blowdownIsolated
      ? '<b>Blowdown is isolated.</b> That was the correct release-control action '
        + 'and it has also removed the sample path, so the monitor that '
        + 'identified the leak is no longer reading. Steam line indication is '
        + 'what remains.'
      : ''}
  </div>`)}
  </div>`;
...
/**
 * Rod control and position indication.
 *
 * Laid out the way the question is asked: the step counters on one side, the
 * measured positions on the other, and the disagreement between them made
 * visible rather than left for the operator to compute. A rod position display
 * that simply prints demand under a DRPI heading would look identical on a
 * healthy core and hide the only fault it exists to reveal.
 */
export function rodcontrol(PL) {
  const R = PL.ro, rp = PL.rop, S = R.stepsPerBank;

  // Per-rod tile: a small vertical gauge, fill proportional to indicated
  // position, so a dropped rod is visible as a shape rather than as a number
  // the eye has to read and compare.
  const rodTile = (rod, i) => {
    const d = R.drpi[i], dev = R.deviation[i];
    const lost = d === null;
    const frac = lost ? 0 : d / S;
    const bad = !lost && d <= rp.rodBottomSteps && rod.demand > rp.rodBottomSteps;
    const warn = !lost && Math.abs(dev) > rp.deviationSteps;
    const cls = lost ? 'rodlost' : (bad ? 'roddrop' : (warn ? 'roddev' : 'rodok'));
    const label = lost ? '——' : String(d);
    return `<div class="rod ${cls}" title="${rod.id}: demand ${rod.demand}, ` +
      `${lost ? 'no indication' : 'DRPI ' + d + ' (' + (dev > 0 ? '+' : '') + dev + ')'}` +
      `${rod.stuck ? ', STUCK' : ''}${rod.dropped ? ', DROPPED' : ''}">
      <div class="rodbar"><i style="height:${(frac * 100).toFixed(1)}%"></i></div>
      <div class="rodnum">${label}</div>
      <div class="rodid">${rod.id.split('-')[1]}</div>
    </div>`;
  };

  const groupBlock = g => {
    const items = R.rods.map((r, i) => [r, i]).filter(([r]) => r.group === g.id);
    return `<div class="rodgroup">
      <div class="rodghead">${g.name}<b>${g.demand} steps</b></div>
      <div class="rodrow">${items.map(([r, i]) => rodTile(r, i)).join('')}</div>
      <div class="rodgfoot">${g.noIndication ? g.noIndication + ' without indication · ' : ''}${
        g.lo === null ? 'no indication' : `indicated ${g.lo}–${g.hi}, spread ${g.spread}`}</div>
    </div>`;
  };
  ...
  ${W.card('Rod control', `<div class="rows">${W.rows([
    ['Mode', PL.rodAuto ? 'AUTOMATIC' : 'MANUAL', PL.rodAuto ? 'good' : 'warn'],
    ['Demand counter', F(PL.banks.ctrlDemand, 0) + ' steps', 'dimv'],
    ['Tavg', F(PL.S.Tavg, 1) + ' °F', 'dimv'],
  ...
  ${W.card('Position indication', `<div class="rows">${W.rows([
    ['Coil spacing', rp.coilSpacing + ' steps', 'dimv'],
    ['Resolution', R.degradedChannels
      ? `±${rp.coilSpacing} steps degraded` : `±${rp.coilSpacing / 2} steps`,
      R.degradedChannels ? 'warn' : 'good'],
    ['Channels degraded', R.degradedChannels + ' of ' + R.rods.length,
      R.degradedChannels ? 'warn' : 'good'],
    ['Rods without indication', R.rods.filter(r => !r.drpiA && !r.drpiB).length,
      R.alarms.drpiLost ? 'bad' : 'good'],
    ['Largest deviation', worstDev === 0 ? 'none'
      : (worstDev > 0 ? '+' : '') + worstDev + ' steps',
      Math.abs(worstDev) > rp.deviationSteps ? 'bad' : 'good']
  ])}</div>`)}

  ${W.card('Rod status', `<div class="rows">${W.rows([
    ['Dropped', R.rods.filter(r => r.dropped).map(r => r.id).join(', ') || 'none',
      R.anyDropped ? 'bad' : 'good'],
    ['Stuck', R.rods.filter(r => r.stuck).map(r => r.id).join(', ') || 'none',
      R.anyStuck ? 'bad' : 'good'],
    ['Slipping', R.rods.filter(r => r.slip).map(r => r.id).join(', ') || 'none',
      R.rods.some(r => r.slip) ? 'warn' : 'good'],
    ['Bank misalignment', R.groups.some(g => g.spread > rp.bankDeviationSteps)
      ? R.groups.filter(g => g.spread > rp.bankDeviationSteps).map(g => g.id).join(', ')
      : 'none', R.alarms.bankMisaligned ? 'bad' : 'good'],
    ['All rods bottomed', PL.trip
  ...
  ${W.card('Rod position indication', `<div class="rodwall">
    ${(R.groups || []).map(groupBlock).join('')}
  </div>`, 'measured, not commanded')}

  ${W.card('Reading this display', `<div class="note">
    The number under each rod is <b>measured</b> position from the coil stack around
    its pressure housing, not the step counter. The counter counts pulses the rod
    control system sent; DRPI observes where the drive shaft actually is, and the
    difference between them is the only way a dropped, stuck or slipped rod is
    found. Indication is quantised to ${rp.coilSpacing} steps because that is the
    coil spacing, so a rod off by less than that is genuinely invisible — the
    deviation alarm at ${rp.deviationSteps} steps sits outside the instrument's own
    resolution for exactly that reason.
    ${R.anyDropped
      ? '<b>A rod is on the bottom.</b> Reactivity is modelled; the radial power '
        + 'tilt it causes is not, because this model has one thermal node and no '
        + 'radial shape for a tilt to exist in. The real DNBR penalty comes from '
        + 'that peaking, so treat the thermal margin shown here as optimistic.'
      : ''}
  </div>`)}`;
}
