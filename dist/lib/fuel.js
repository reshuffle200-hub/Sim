...
export function dnbr(P, p, flowFr, Ppsia, TinF) {
  // Tsat was being evaluated three separate times here, once for each of the
  // three saturation properties, on a function called every step.  props.js
  // tabulates all three against pressure to within 0.03%, which is far inside
  // the correlation uncertainty of the DNBR fit they feed.
  const Pm = ST.MPa_from_psi(Ppsia);
  const Ts = PR.TsatP(Ppsia);
  const hfSat = PR.hfP(Ppsia);
  const hgSat = PR.hgP(Ppsia);
