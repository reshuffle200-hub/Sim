...
function refresh(P, S) {
  let alphaMax = 0;
  for (const n of S.nodes) {
    const st = stateOf(S.P, n.U / Math.max(n.M, 1e-9));
    n.h = st.h; n.T = st.T; n.x = st.x; n.void = st.alpha;
    n.rho = 1 / st.v;
    if (!n.isPzr) alphaMax = Math.max(alphaMax, st.alpha);
  }
  const Tsat = stateOf(S.P, S.nodes[S.idx.CR].U / Math.max(S.nodes[S.idx.CR].M, 1e-9)).Tsat;
  S.Tsat = Tsat;
...
export function stepRCS(P, S, Qcore, dt, opt = {}) {
  ...
  if (S.breakArea > 0) {
    ...
    // subcooled -> orifice; two-phase -> reduced critical flux
    const G = n.void > 0.02
      ? 4500 * Math.sqrt(Pd) * (1 - 0.55 * n.void)
      : 8000 * Math.sqrt(Pd);
