...
export function stepSI(P, S, Ppsia, dt, opt = {}) {
  ...
  if (S.actuated && S.rwstPct > P.rwstEmptyPct) {
    for (let i = 0; i < P.hhTrains; i++) if (S.hhOn[i]) hh += pumpFlow(P.hhMaxGpm, P.hhShutoffPsig, psig);
    for (let i = 0; i < P.siTrains; i++) if (S.siOn[i]) si += pumpFlow(P.siMaxGpm, P.siShutoffPsig, psig);
    for (let i = 0; i < P.lhTrains; i++) if (S.lhOn[i]) lh += pumpFlow(P.lhMaxGpm, P.lhShutoffPsig, psig);
  }
