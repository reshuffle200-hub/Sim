 ...
 */
export function stepRHR(P, R, Ppsia, ThotF, TavgF, Mrcs, dt, opt = {}) {
  const psig = Ppsia - 14.7;

  // Component cooling water is a real loop now (ccw.js), so when the plant
  // supplies its temperature, use it.  The self-computed value below is kept
  // only so this module still runs standalone in its own test, where there is
  // no CCW loop to ask -- but a self-computed sink is exactly the asserted
  // boundary condition ccw.js exists to remove, and the plant never uses it.
  if (opt.ccwTempF !== undefined) R.ccwTempF = opt.ccwTempF;
  ...
  // --- CCW, only when nobody better is supplying it ---
  if (opt.ccwTempF === undefined) {
