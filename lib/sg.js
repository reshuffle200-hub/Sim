...
export function sgParams() {
  const P = {
    ...
    arvLiftPsig: 1025,     // atmospheric relief (about 10% of steam flow)
    arvCapLbHr: 4.2e5
  };
  calibrateSG(P);
  return P;
...
export function riserVoidFrom(P, QinW, Wfw, hfw, Psec, Wsteam) {
  ...
  const Wboil = Qboil / hfg;                       // steam produced, lbm/hr
  const Wcirc = Math.max(P.circRatio * Wboil, 1e4);
  const xExit = clamp(Qboil / (Wcirc * hfg), 1e-5, 0.95);
