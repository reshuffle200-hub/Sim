...
export function secParams() {
  const P = {
    ...
    tauDump: 2.0,
    condenserOK: true,
    dumpInterlockPsia: 8.0,    // dumps blocked above this condenser pressure
...
export function calibrateSec(P, WsteamTotal) {
  ...
  P.Kturb = P.Wrated / P.Prated;                       // choked: W = K * Pos * P
...
export function makeSec(P) {
  return {
    ...
    // misc
    condPsia: P.condPsia, runback: false
  };
}
...
export function stepSec(P, S, sgs, Tavg, Tref, dt) {
  ...
  if (S.tripped) { S.loadDemand = 0; S.valveDemand = 0; }
    ...
    S.valveDemand = clamp(S.loadDemand * P.Prated / Math.max(S.Pheader, 50), 0, 1);
  ...
  S.MWe = S.Wturb * P.dhEff / 3.412142 / 1e6;
  ...
  S.condDutyMW = (S.Wturb * (hg - PR.hfT(P.hotwellF)) * 0.62 + S.Wdump * (hg - PR.hfT(P.hotwellF)))
                 / 3.412142 / 1e6;
  return S;
}
