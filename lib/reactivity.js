...
function calibrate(P) {
  ...
  st.banks.ctrlDemand = 4 * P.stepsPerBank - 3 * (P.stepsPerBank - P.overlap) - 8;
  P.excess = 0;
...
export function fullOutBanks(P) {
  return {
    ctrlDemand: 4 * P.stepsPerBank - 3 * (P.stepsPerBank - P.overlap),  // 528
...
export function total(P, st) {
  ...
  const rods = rodWorth(P, st.banks);
