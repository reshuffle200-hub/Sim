...
function lerp1(tbl, p) {
  const fi = iOfP(p), i0 = Math.min(NP - 2, Math.floor(fi)), a = fi - i0;
  return (1 - a) * tbl[i0] + a * tbl[i0 + 1];
}
function lerp2(tbl, p, f) {
  const fi = iOfP(p), i0 = Math.min(NP - 2, Math.floor(fi)), a = fi - i0;
  const fj = Math.min(Math.max(f, 0), 1) * (NS - 1);
  const j0 = Math.min(NS - 2, Math.floor(fj)), b = fj - j0;
  const k0 = i0 * NS + j0, k1 = k0 + NS;
  return (1 - a) * ((1 - b) * tbl[k0] + b * tbl[k0 + 1])
       +      a  * ((1 - b) * tbl[k1] + b * tbl[k1 + 1]);
}
...
export function vPH(p, h) {
  const hf = hfP(p), hg = hgP(p);
  if (h <= hf) return lerp2(SUB_V, p, hf > 0 ? h / hf : 0);
  if (h >= hg) return lerp2(SUP_V, p, Math.sqrt((h / hg - 1) / SUP_SPAN));
  const x = (h - hf) / (hg - hf);                    // two-phase: analytic
  return vfP(p) + x * (vgP(p) - vfP(p));
}
/** Temperature [degF] from pressure and enthalpy. */
export function TPH(p, h) {
  const hf = hfP(p), hg = hgP(p);
  if (h <= hf) return lerp2(SUB_T, p, hf > 0 ? h / hf : 0);
  if (h >= hg) return lerp2(SUP_T, p, Math.sqrt((h / hg - 1) / SUP_SPAN));
  return TsatP(p);
}
...
export function alphaPH(p, h) {
  ...
  if (h <= hf) return 0;
  if (h >= hg) return 1;
  const x = (h - hf) / (hg - hf), vf = vfP(p), vg = vgP(p);
  const v = vf + x * (vg - vf);
